...
public class Client {
	...
	private static int totalNum = 0; // 총 참가하고 있는 인원 수
	private static Vector<String> names = new Vector<String>();

	private static String liarName; // 매 판마다 바뀌는 라이어 이름
	private static int gameClient = 0; // 게임에서 활용되는 투표 인원 수 등
	private static String wordTopic; // 선정된 주제

	private static Hashtable<String, Integer> mp = new Hashtable<>(); // 투표의 개수를 헤아리기 위함
	private MyLiarBiz myBiz = new MyLiarBiz(); // Biz 함수들 들고 오기 위함.

	public static boolean roomAvailable = true; // 현재 방 들어올 수 있는지
			...
			public void run() {
				try {
					while (true) {
						...
						// 메세지를 정상적으로 받은 경우
						if (totalNum >= 9) {
							sendLogin(Classifiers.ServerNotAvailable);
						} else if (roomAvailable == false) {
							sendLogin(Classifiers.ServerNotAvailable);
						} else {
							// 메세지를 정상적으로 받은 경우
							...
							String[] array = message.split(Classifiers.loginClassifier);
							id = array[0];
							password = array[1];

							boolean check = myBiz.findByUserId(id);// 닉네임 검사 함수 info는 userConnectInfo class에서 확인 가능
							if (check == true) {// 생성 가능한 경우
								sendLogin(Classifiers.CreateAvailableClassifier);
								System.out.println("[ID생성 가능]");
								myBiz.addUser(id, password);
								// 채팅 소켓으로 값이 보내진다.
								...
								sendLogin(Classifiers.CreateNotAvailableClassifier);
			...
			public void run() {
				try {
					while (true) {
						...
						String message = new String(buffer, 0, length, "UTF-8");
						if (message.equals(Classifiers.GameStartClassifier)) {
							roomAvailable = false;
							...
							myBiz.deleteUser(message);
							totalNum--;
							if(gameClient>0 && gameStarted==true)
								gameClient--;
							Main.clients.remove(Client.this);
							...
							String chatMessage = Integer.toString(totalNum) + "::" + message + "님이 퇴장하셨습니다. "
									+ nameMessage;
							sendAllClient(chatMessage);
							makeTerm(100);
							if(gameClient==0 && gameStarted==true) {
	...
	public List<String> liarSelect() {
		...
		List<MyLiarVo> all = myBiz.select_all_liar();
		gameStarted=true;
		...
		while (iterator.hasNext()) {
			...
			client.sendChat(Classifiers.GameStartReturnClassifier);
			makeTerm(100);
			client.sendChat("\n게임이 곧 시작됩니다.\n");
			makeTerm(100);
	...
	public void sendAllClient(String chatMessage) {
	...
	public void endVote() {
		...
		sendAllClient(Classifiers.LiarClassifier + Boolean.toString(LiarWin) + "$" + "라이어는 <'"
				+ liarName + "'> 님이었습니다.");

		roomAvailable = true;
		gameStarted=false;
	}
	public void makeTerm(int sec) {
		try {
