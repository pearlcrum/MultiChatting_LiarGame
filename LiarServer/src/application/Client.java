					...
					while (true) {
						...
						if (message.equals("게임시작")) {
							...
							for(int i=0; i<names.size();i++)
							{
								...
								{
									System.out.println("여기서 걸리는 것 같은데");
									info.vec.set(i, info.vec.get(i)+1);//투표용 set
								}
							}
							gameClient--;
							...
							if(gameClient==0)
							{
								...
								for(int i=0; i<info.vec.size();i++) {
									if(info.vec.get(i)>max)
									{
										max=info.vec.get(i);
										votedLiar=i; //가장 많이 뽑힌 사람
									}
									else if(info.vec.get(i)==max)
									{
										votedLiar=-1;
								...
								if(votedLiar<0){
									//음수일 경우 Liar 승리
									for(Client client: Main.clients) {
										client.sendChat("투표에서 동점이 나와 아무도 라이어로 선정되지 않아, 라이어가 승리했습니다.");
									}
								}else {
									//가장 많이 뽑힌 사람의 index는 votedLiar, names에서 찾으면 된다.
									if(liarName.equals(names.get(votedLiar)))
									{
										for(Client client: Main.clients) {
											client.sendChat("라이어는"+names.get(votedLiar)+"님이었으며, 시민들의 승리 입니다.");
										}
									}
									else
									{
										for(Client client: Main.clients) {
											client.sendChat("라이어는"+names.get(votedLiar)+"님이었으며, 라이어의 승리 입니다.");
										}
									}
								}
